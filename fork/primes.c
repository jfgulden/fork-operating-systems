...
buscar_primos(int p_lectura)
{
	...
	if (r > 0) {
		...
		if (f > 0) {
			close(fds[0]);
			while ((r = read(p_lectura,
			                 &valor_a_enviar,
			                 sizeof(valor_a_enviar))) != 0) {
				if (valor_a_enviar % primo != 0) {
					// Escribe sobre el extremo del nuevo pipe
					w = write(fds[1],
					          &valor_a_enviar,
					          sizeof(valor_a_enviar));
					if (w < 0) {
						mostrar_error_write();
						exit(ERROR);
					}
...
main(int argc, char *argv[])
{
	...
	if (argc != ARGS_MIN) {
		printf("Debe proporcionar un número entero en la línea de "
		       "ejecución.\n");
		exit(ERROR);
	}
	...
	if (f == 0) {
		...
		if (resultado_busqueda == ERROR) {
			printf("Error al intentar buscar los números primos de "
			       "%i\n",
			       numero);
			exit(ERROR);
		}
