...
bool es_directorio_actual_especial(char* nombre){
    return (strcmp(nombre, DIR_ACTUAL) == 0 || strcmp(nombre, DIR_SUPERIOR) == 0);
}

int buscar_archivo(DIR *dir, char path_actual[], char archivo_buscado[], char* (*comparar_substring)(const char*, const char*)){
    struct dirent *acceso_dir = readdir(dir);
    int fd_acceso_dir = dirfd(dir);
    if (fd_acceso_dir == ERROR){
        printf("Error al intentar obtener el file descriptor del directorio\n");
        return ERROR;
    }
    ...
    while(acceso_dir != NULL){
        ...
        if (comparar_substring(acceso_dir->d_name, archivo_buscado) != NULL){
        ...
        if (acceso_dir->d_type == DT_DIR){
            ...
            int fd_acceso_subdir = openat(fd_acceso_dir, acceso_dir->d_name, O_DIRECTORY);
            if (fd_acceso_dir == ERROR){
                printf("Error al intentar abrir el subdirectorio\n");
                return ERROR;
            }
            DIR *subdir = fdopendir(fd_acceso_subdir);
            if (!subdir){
                printf("Error al intentar obtener acceso al subdirectorio\n");
                return ERROR;
            }
            int resultado_busqueda = buscar_archivo(subdir, subdir_path, archivo_buscado, comparar_substring);
...
int main(int argc, char *argv[]){
    ...
    resultado_busqueda = buscar_archivo(dir, path, archivo_buscado, comparar_substring);
