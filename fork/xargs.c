...
void liberar_args(char *args[TAM_ARRAY], int tope){
    for (int i = 1; i < (tope-1); i++){     //rgs[tope] = NULL. Por eso libero hasta tope-1
        free(args[i]);
        args[i] = NULL;
    }
}
int ejecutar_comando(char *args[TAM_ARRAY], int* tope){
    args[NARGS+1] = NULL;
    (*tope)++;
...
int main(int argc, char *argv[]) {
    ...
    size_t tam = 0;
    char *linea = NULL;
    char *args[TAM_ARRAY] = {argv[1]};  //TAM_ARRAY = 6 porque en la última posición va NULL y en la primera va argv[1] -> ls
    int tope = 1;
